...
--if not opt then
   ...
   cmd:option('full', true, 'full dataset or partial')
   cmd:option('coefL1', 0, 'L1 penalty on the weights')
   cmd:option('coefL2', 1e-3, 'L2 penalty on the weights')
   ...
   cmd:option('norm', 'l_1', 'uncertainty l-ball: l_inf | l_2 | l_1')
...
-- read function that generates adversarial examples
dofile 'computeAdversarials.lua'
...
function train(dataset)
   ...
   for t = 1,dataset:size(),opt.batchSize do
      ...
      inputs = adversarial_fast(model, criterion, inputs, targets, 1, opt.intensity, opt.norm)  
   ...
   local filename = paths.concat(opt.save, 'mnist.l_1.BN.net')
...
-- this network has test accuracy of 99.12%, with intensity of 2
